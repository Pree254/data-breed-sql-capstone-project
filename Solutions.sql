...
-- Task 1: Retrieve all customers from the United States (country_code = 'US') sorted by 
...
-- Using subquery/CTE to determine the last 30days
...
-- Task10.Find customers who have made bookings but never placed an order, using a set operation
...
order_id
FROM customers c
LEFT JOIN bookings b ON c.customer_id = b.booking_customer_id
LEFT JOIN orders o ON c.customer_id = o.order_customer_id
WHERE booking_customer_id is NOT NULL;

SELECT DISTINCT b.booking_customer_id AS customer_id
FROM bookings b
LEFT JOIN orders o ON b.booking_customer_id = o.order_customer_id
WHERE o.order_customer_id IS NULL;

SELECT DISTINCT b.booking_customer_id AS customer_id
FROM bookings b
WHERE b.booking_customer_id IS NOT NULL
  AND b.booking_customer_id NOT IN (
    SELECT DISTINCT o.order_customer_id
    FROM orders o
...
GROUP BY 
    s.staff_id, s.staff_first_name, s.staff_last_name;
  
-- Task 12. Write a stored procedure that takes a date range and returns the busiest times of day
-- (based on booking counts) during that period

DELIMITER $$

CREATE PROCEDURE GetBusiestBookingTime()
BEGIN
...
CALL GetBusiestBookingTime;

-- Task 13. Create a triggerthat automatically updates the delivery status to 'Preparing'
...
WHERE DAYOFWEEK(booking_date) IN (6, 7)  
  AND TIME(booking_time) >= '18:00:00';

-- Task 15. Extract the year and month from order dates and group sales by these periods.
SELECT 
YEAR( order_date) AS order_year,
MONTH(order_date)AS order_month,
SUM(sales) OVER(PARTITION BY YEAR (order_date)ORDER BY MONTH (order_date)) AS monthly_sales
FROM orders;
...
-- Breaks down complex logic into named steps (`first_orders`, `labeled_orders`) 
-- Avoids repeating the same subquery for each row — CTE evaluated once          
-- Easier to reuse intermediate results and debug parts independently            
-- CTEs support recursion (not needed here, but useful in hierarchical data)     
-- Much cleaner when collaborating or making changes later                      
    ...
    -- Base case: Start with all starters
    SELECT 
        s.name AS starter,
...
-- Task 23. Analyze sales growth by comparing each month's 
-- sales to the same month in the previous year using window functions.
SELECT 
...
SUM(sales)-LAG(SUM(sales)) OVER (PARTITION BY YEAR(order_date) ORDER BY MONTH(order_date) ROWS BETWEEN 1 PRECEDING AND CURRENT ROW) AS monthly_sales_growth
FROM orders
GROUP BY YEAR(order_date), MONTH(order_date)
...
monthly_customers AS (
    ...
    FROM orders o
    JOIN first_orders f ON o.order_customer_id = f.order_customer_id
)
SELECT 
    order_year,
    order_month,
...
GROUP BY order_year, order_month, customer_type
ORDER BY order_year, order_month, customer_type;
...
ORDER BY total_profit DESC;
 ...
 SUM(sales) AS total_sales
FROM staff s
JOIN orders o ON s.staff_id = o.order_staff_id
GROUP BY s.staff_id
...
-- Task 28: Create a stored procedure called CheckBooking to check whether a table in the restaurant is already booked. The procedure should have two input parameters in the form of booking date and table number.
...
SELECT * FROM bookings;        USE LittleLemonDB;
...
-- Task 30 Create a new procedure called AddBooking to add a new table booking record
...
-- Task 31 Create a new procedure called UpdateBooking that they can use to update existing bookings in the booking table
...
-- Task 32. Create a new procedure called CancelBooking that they can use to cancel or remove a booking.
